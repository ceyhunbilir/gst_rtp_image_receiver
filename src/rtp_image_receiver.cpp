...
RtpImageReceiverNode::RtpImageReceiverNode(ros::NodeHandle& nh,
  ...
  std::string camera_info_url =
      private_nh_.param("camera_info_url", std::string(""));
  ...
  if (publish_raw_ && publish_compressed_) {
    ...
    ROS_ERROR(
        "Both publish_raw and publish_compressed are false. "
        "No data will be received. Defaulting to RAW_ONLY.");
    config_.mode = SinkMode::RAW_ONLY;
  }
  ...
  stats_timer_ = nh_.createTimer(
      ros::Duration(5.0), &RtpImageReceiverNode::PublishStatistics, this);
...
std_msgs::Header RtpImageReceiverNode::CreateHeader(
...
void RtpImageReceiverNode::PublishRaw(
    const uint8_t* data, size_t size, const std_msgs::Header& header) {
  if (publish_raw_ && raw_pub_.getNumSubscribers() > 0) {
    cv::Mat bgr_image(config_.height, config_.width, CV_8UC3,
                      const_cast<uint8_t*>(data));

    sensor_msgs::ImagePtr image_msg =
        cv_bridge::CvImage(header, "bgr8", bgr_image).toImageMsg();
    ROS_INFO("Publish Image topic          : %09u.%09u",
...
void RtpImageReceiverNode::PublishCompressed(
    const uint8_t* data, size_t size, const std_msgs::Header& header) {
  if (publish_compressed_ && compressed_pub_.getNumSubscribers() > 0) {
    ...
    ROS_INFO("Publish CompressedImage topic: %09u.%09u",
             header.stamp.sec, header.stamp.nsec);
  }
}

void RtpImageReceiverNode::PublishCameraInfo(
...
void RtpImageReceiverNode::JpegCallback(
...
void RtpImageReceiverNode::RawCallback(
    const uint8_t* data, size_t size,
    const ImageReceiver::RTPTimestamp& timestamp) {
  std_msgs::Header header = CreateHeader(timestamp);
...
void RtpImageReceiverNode::CombinedCallback(
    const uint8_t* raw_data, size_t raw_size,
    const uint8_t* jpeg_data, size_t jpeg_size,
    const ImageReceiver::RTPTimestamp& timestamp) {
  std_msgs::Header header = CreateHeader(timestamp);

  // Publish both data streams, which are synchronized by PTS
  // within ImageReceiver::Impl
...
void RtpImageReceiverNode::PublishStatistics(const ros::TimerEvent&) {
  ...
  ROS_INFO(
      "Stats : Frames(Raw)=%lu, Dropped(Raw)=%lu, "
      "Frames(Jpeg)=%lu, Dropped(Jpeg)=%lu, FPS=%.2f",
